...
public class Broker {
    ...
    public static Broker createBrokerFormJSON(JSONObject o) throws JSONException {
        Broker broker = new Broker();
        if (o.has("pushserver")) {
            broker.pushserver = o.getString("pushserver");
        }
        broker.uri = o.getString("uri");
        broker.user = o.getString("user");
        String p = o.getString("password");
        if (p != null) {
            broker.password = p.toCharArray();
        }
        if (o.has("clientID"))
            broker.clientID = o.getString("clientID");
        JSONArray t = o.getJSONArray("topics");
        for(int i = 0; i < t.length(); i++) {
            broker.topics.add(t.getString(i));
        }
        return broker;
    }

    // order by PushServer, MQTT Server, user name
    public static class Comparator implements java.util.Comparator<Broker> {
